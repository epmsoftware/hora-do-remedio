    ...
    if (err) {
        ...
        db.run(`
            CREATE TABLE IF NOT EXISTS pacientes (
                ...
                descricao TEXT                        -- Descrição ou observações gerais
            )
        `);
        ...
        db.run(`
            CREATE TABLE IF NOT EXISTS cadastromedicamentos (
                ...
                usuario_id INTEGER,                   -- Chave estrangeira que liga ao usuário que cadastrou
                FOREIGN KEY (usuario_id) REFERENCES usuarios(id) -- Define o relacionamento com a tabela 'usuarios'
            )
        `);
    }
});
